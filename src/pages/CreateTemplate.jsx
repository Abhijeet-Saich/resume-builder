...
import { serverTimestamp, deleteDoc, setDoc, doc } from "firebase/firestore";
import { initialTags, adminIds } from "../utils/helper";
...
const CreateTemplate = () => {
  ...
  const pushToCloud = async () => {
    ...
    if(!_doc.tags.length){
      toast.info("Kindly select tags")
    }else if(!formData.title){
      toast.info("Kindly fill title Template title")
    }else if(!imageAsset.uri){
      toast.info("Kindly upload the image")
    }else{
  ...
  const removeTemplate = async (template) => {
    ...
    await deleteObject(deleteRef).then(async () => {
      await deleteDoc(doc(db, "templates", template?._id))
        .then(() => {
          toast.warning("Template deleted from the cloud");
          templatesRefetch();
        })
        .catch((err) => {
    ...
    <div className="w-full px-4 lg:px-10 2xl:px-32 py-4 grid grid-cols-1 lg:grid-cols-12">
      ...
      <div className="col-span-12 lg:col-span-4 2xl:col-span-3 w-full flex-1 flex items-center justify-start flex-col gap-4 px-2">
        ...
        <div className="w-full flex items-center justify-end">
          ...
          {/* <p className="text-sm text-txtDark capitalize font-bold">{templates && templates?.length > 0 ? `${templates?.length+1}` : `${templates?.length}`}</p> */}
          <p>{templates?.length + 1}</p>
        </div>
              ...
              {!imageAsset?.uri ? (
                ...
                <React.Fragment>
                  <div className="w-full h-full rounded-md overflow-hidden relative">
                    ...
                    {/* delete section */}
                    <div
                      className="absolute top-4 right-4 w-8 h-8 flex items-center rounded-md bg-red-500 cursor-pointer justify-center"
        ...
        <button
          className="w-full rounded-md py-3 text-white bg-blue-700"
          onClick={pushToCloud}
        >
          Save
        </button>
