...
const Filters = () => {
  ...
  const { data: filterData, isError , isLoading } = useFilters();
  ...
  const handleFilterValue = (value) => {
    queryClient.setQueryData(
      "globalFilter",
      { ...queryClient.getQueryData("globalFilter"), searchTerm : value }
    );
  };
  ...
  return (
    <div className="w-full flex items-center justify-start py-4">
      ...
      <div
        ...
        onClick={clearFilter}
      >
        <MdLayersClear clear="true" className="text-xl" />
        <AnimatePresence>
          {isClearHovered && (
            <motion.div
        ...
        {FiltersData &&
          FiltersData?.map((item) => (
            <div
              ...
              key={item.id}
              className={`border border-gray-300 rounded-md px-6 py-2 cursor-pointer group-hover:shadow-md ${filterData.searchTerm === item.value && "bg-blue-300 shadow-md"}`}>
              <p className="text-sm text-txtPrimary group-hover:text-txtDa whitespace-nowrap">
                {item.label}
              </p>
            </div>
          ))}
